---
name: email-campaign-ben-hawksworth
description: Use when generating email campaigns for The Coach Consultant, especially under time pressure or when voice matching and performance optimization are critical
---
...
```dot
digraph voice_workflow {
    "Start email generation" [shape=doublecircle];
    "Read voice-characteristics.md" [shape=box];
    "Check forbidden-phrases list" [shape=box];
    "Write email 1" [shape=box];
    "Run forbidden phrase scan" [shape=box];
    "Contains forbidden?" [shape=diamond];
    "Add Ben's natural phrases" [shape=box];
    "Verify tone" [shape=box];
    "Score voice (1-10)" [shape=box];
    "Score >= 8?" [shape=diamond];
    "Rewrite problem sections" [shape=box];
    "More emails needed?" [shape=diamond];
    "Write next email" [shape=box];
    "Deliver all emails" [shape=box, style=filled, fillcolor=lightgreen];

    "Start email generation" -> "Read voice-characteristics.md";
    "Read voice-characteristics.md" -> "Check forbidden-phrases list";
    "Check forbidden-phrases list" -> "Write email 1";
    "Write email 1" -> "Run forbidden phrase scan";
    "Run forbidden phrase scan" -> "Contains forbidden?";
    "Contains forbidden?" -> "Rewrite problem sections" [label="yes"];
    "Contains forbidden?" -> "Add Ben's natural phrases" [label="no"];
    "Rewrite problem sections" -> "Run forbidden phrase scan";
    "Add Ben's natural phrases" -> "Verify tone";
    "Verify tone" -> "Score voice (1-10)";
    "Score voice (1-10)" -> "Score >= 8?";
    "Score >= 8?" -> "Rewrite problem sections" [label="no"];
    "Score >= 8?" -> "More emails needed?" [label="yes"];
    "More emails needed?" -> "Write next email" [label="yes"];
    "More emails needed?" -> "Deliver all emails" [label="no"];
    "Write next email" -> "Run forbidden phrase scan";
}
```
